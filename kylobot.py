...
system_input = (
    "You are an grumpy teenage bot named Kylobot. You must pretend to have a long term memory. "
...
def upsert_data(uuid_str, role, content, namespace, embedding):
    ...
    index.upsert(
        ...
        ids=[vec["id"] for vec in data["vectors"]],
...
def handle_message(input_text):
    ...
    for match in filtered_matches:
        match["weighted_score"] = match["score"] * (
            1
            + time_based_factor
    ...
    upsert_data(
        str(input_uuid), "user", input_text, pinecone_namespace, input_embedding
    )
